...
void DragMeterScreen::onShow() {
  ...
  if (targetIdx >= 0 && targetIdx < 10) _targetTime = targets[targetIdx];
  else _targetTime = 10.0;
  
  _referenceTime = p.getFloat("drag_ref", 0.0);
  p.end();

  if (treeIdx <= 5) _treeInterval = (treeIdx + 1) * 500;
  else if (treeIdx == 6) _treeInterval = 4000;
  else _treeInterval = 5000;
  
  _displayMode = DISPLAY_NORMAL;
  _predictedFinalTime = 0;
  
  _summaryShowBest = false;
  _sessionBest.clear(); // Or keep session best? "Best in session" usually implies since power on or manual reset.
  // Actually, SessionManager should handle this, but for now we keep it local.
  // If we want it to persist across Screen Switches, we shouldn't clear it in onShow unless it's a new "Session".
  // Let's NOT clear _sessionBest here to allow switching screens.
...
void DragMeterScreen::checkStartCondition() {
    ...
    if (speed > 1.0) { // Moving (> 1 km/h)
        if (_rolloutEnabled) {
            ...
            if (dt > 1.0) dt = 0.1; // Cap dt if weird jump
...
void DragMeterScreen::drawPredictiveMode() {
    ...
    if (_targetTime > 0 && _predictedFinalTime > 0) {
        ...
        if (abs(delta) <= 0.1) color = TFT_GREEN;
        else color = TFT_RED;
...
void DragMeterScreen::checkStopCondition() {
    ...
    if (gpsManager.getSpeedKmph() < 1.0) {
        ...
        if (actualFilename.length() == 0) actualFilename = "DragRun"; // Fallback
        ...
        sessionManager.appendToHistoryIndex(actualFilename, dateStr, 1, resultTime, "DRAG");
...
void DragMeterScreen::updateDisciplines() {
    ...
    if (dt > 0.5) dt = 0.1; // Cap
    lastUpdate = now;
    
    if (_runState == RUN_RUNNING && runTime < 100) _totalRunDistance = 0; // Reset at start
...
void DragMeterScreen::drawChristmasTreeOverlay() {
    ...
    // Yellows
    if (phase >= 1) tft->fillCircle(cx, cy + 20, 20, TFT_YELLOW);
    if (phase >= 2) tft->fillCircle(cx, cy + 60, 20, TFT_YELLOW);
    if (phase >= 3) tft->fillCircle(cx, cy + 100, 20, TFT_YELLOW);
...
void DragMeterScreen::handleMenuTouch(int idx) {
    ...
    if (item == "DRAG SCREEN") {
        ...
        _ui->getTft()->fillScreen(COLOR_BG);
        drawDragModeMenu();
    } else if (item == "PREDICTIVE" || item == "SUMMARY") {
        _state = STATE_SUMMARY_VIEW;
        _ui->getTft()->fillScreen(COLOR_BG); // Clear for summary
        drawDashboardStatic(); // drawDashboardStatic calls drawSummary
    }
}
...
void DragMeterScreen::loadDisciplines(int modeIdx) {
    ...
    if (modeIdx == 0) {
        ...
        // Fields: name, isDist, start, target, resTime, compl, endSpd, slope, peakSpd, brakeDist, valid
        _disciplines.push_back({"0-60", false, 0, 60, 0, false, 0, 0, 0, 0, true});
        _disciplines.push_back({"0-100", false, 0, 100, 0, false, 0, 0, 0, 0, true});
        _disciplines.push_back({"100-200", false, 100, 200, 0, false, 0, 0, 0, 0, true});
        _disciplines.push_back({"0-200", false, 0, 200, 0, false, 0, 0, 0, 0, true});
        ...
        _disciplines.push_back({"60 ft", true, 0, 18.288, 0, false, 0, 0, 0, 0, true});
        ...
        // CUSTOM (Placeholder)
        _disciplines.push_back({"Custom 1", true, 0, 100, 0, false, 0, 0, 0, 0, true});
        _highlightTitle = "Custom";
        _highlightValue = "--.--";
    }
    
    // Reset session best if structure changed? 
    // Ideally we match by name. For simplicitly, clear session best on mode change.
    _sessionBest.clear();
}
...
void DragMeterScreen::drawDashboardStatic() {
  ...
  if (_state == STATE_MENU) {
      ...
      // Header Horizontal Line already drawn at y=45 by drawFastHLine in common area if we move it? 
      // Actually drawDashboardStatic has "Back" button which uses y=25.
      // We should draw a line below the header.
      ...
      tft->drawFastVLine(SCREEN_WIDTH / 2, 110, SCREEN_HEIGHT - 110 - 20, TFT_WHITE); // -20 for footer
      ...
      // We will draw this in Dynamic to avoid flickering or redraw it here and update text in Dynamic?
      // Better to draw static bg here.
      tft->fillRect(SCREEN_WIDTH / 2 + 1, 111, SCREEN_WIDTH / 2 - 1, SCREEN_HEIGHT - 110 - 21, TFT_WHITE);

      // Footer Area (Slope)
      tft->fillRect(0, SCREEN_HEIGHT - 20, SCREEN_WIDTH, 20, COLOR_BG); // Clear footer
      tft->setTextColor(TFT_WHITE, COLOR_BG);
      tft->setTextDatum(TL_DATUM);
...
void DragMeterScreen::drawSummary() {
    ...
    // We captured peakSpeed in each discipline. Let's show the peak of the last/longest one.
    float peak = 0;
    const std::vector<Discipline>* data = _summaryShowBest ? &_sessionBest : &_disciplines;
    ...
    float brakeDist = 0;
    if (!data->empty()) brakeDist = data->back().brakingDistance;
    
    tft->setTextDatum(TR_DATUM);
    tft->drawString(String(brakeDist, 1) + " m", SCREEN_WIDTH - 10, SCREEN_HEIGHT - 20);
...
void DragMeterScreen::handlePredictiveTouch(int idx) {
    ...
    // For now, let's just go to Summary View regardless, or stay in menu with selection indicator?
    // Let's assume selecting it goes to the view for now, as that's typical.
    // Or maybe it toggles a mode and goes back?
    // Given the previous "Predictive" went to "Summary View", let's make "Predictive Mode" go there.
    // "Normal Mode" might just go back to Drag Mode or similar?
