...
void MenuScreen::update() {
  ...
  if (p.x != -1) {
      if (_touchStartY == -1) _touchStartY = p.y;
      ...
      // Tap Detection
          ...
          // Removed Top Arrow Logic as we now use a single toggle button at the bottom
  ...
  if (enter) {
    ...
    case 0: _ui->switchScreen(SCREEN_LAP_TIMER); break;
    case 1: _ui->switchScreen(SCREEN_DRAG_METER); break;
    case 2: _ui->switchScreen(SCREEN_RPM_SENSOR); break;
    case 3: _ui->switchScreen(SCREEN_HISTORY); break;
    case 4: _ui->switchScreen(SCREEN_SETTINGS); break;
    }
  }
...
void MenuScreen::drawMenu(bool force) {
  ...
  if (endIndex > MENU_ITEMS) endIndex = MENU_ITEMS;
  ...
  for (int i = 0; i < (endIndex - startIndex); i++) {
    ...
    bool stateChanged = (actualIndex == _selectedIndex || actualIndex == _lastSelectedIndex);
